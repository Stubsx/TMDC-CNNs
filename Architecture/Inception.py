...
class InceptionModule(nn.Module):
    def __init__(self, num_input_features, num_output_feature, bn_size, drop_rate):
        super(InceptionModule,self).__init__()
        each_planes = num_output_feature // (1 + 3 + 5)
        planes_7x = each_planes
        planes_5x = each_planes * 3
        planes_3x = num_output_feature - planes_5x - planes_7x
...
class InceptionBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        super(InceptionBlock, self).__init__()
        for i in range(num_layers):
            layer = InceptionModule(num_input_features + i * growth_rate, num_input_features + (i+1) * growth_rate, bn_size, drop_rate)
            self.add_module('Inception_module%d' % (i + 1), layer)
...
class Inception(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.block1 = InceptionBlock(num_layers=block_config[0], num_input_features=num_features,
        ...
        self.block2 = InceptionBlock(num_layers=block_config[1], num_input_features=num_features,
                                  bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate)
        ...
        self.block3 = InceptionBlock(num_layers=block_config[2], num_input_features=num_features,
                                  bn_size=bn_size, growth_rate=growth_rate, drop_rate=drop_rate)
